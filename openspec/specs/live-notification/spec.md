...
- **THEN** the system SHALL send a `MessageChain` containing text (🟢, name, title, category, link) and `Comp.Image.fromURL(image_url)` to the group's `unified_msg_origin`
